...
export default function Reports() {
  ...
  const fetchLiveDatabaseReport = async (timeframe) => {
    ...
    setErrorMessage('');
    try {
      const response = await axios.get(`http://localhost:5000/api/admin/daily-report?timeframe=${timeframe}`);
  ...
  // Automatically refresh data when owner toggles Daily, Weekly, or Monthly tabs
  useEffect(() => {
    fetchLiveDatabaseReport(reportTimeframe);
  }, [reportTimeframe]);

  // Helper function to dynamically change sub-headers based on selection
  const getTimeframeLabel = () => {
    if (reportTimeframe === 'daily') return "Today's Operational Balance";
    if (reportTimeframe === 'weekly') return "Current Week's Operational Balance";
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight flex items-center gap-2">
            <BarChart3 className="text-amber-600 w-6 h-6" /> Store Performance Ledger
          </h2>
          <p className="text-sm text-gray-500 mt-1 flex items-center gap-1.5">
            <Calendar size={14} className="text-amber-600" /> Granular overview covering all bookstore catalog items.
          </p>
        </div>
      ...
      <div className="space-y-6 bg-gray-50 p-6 rounded-2xl border border-gray-100">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Live totals for books, stationeries, and all user items.</p>
          </div>
          
          <div className="flex items-center gap-3 w-full sm:w-auto">
            {/* UPDATED: Fully rendering all 3 button interfaces */}
            <div className="flex bg-gray-100 p-1 rounded-lg text-xs font-semibold space-x-1">
              <button 
                onClick={() => setReportTimeframe('daily')}
                className={`px-3 py-2 rounded-md transition-all cursor-pointer ${reportTimeframe === 'daily' ? 'bg-white text-amber-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
              >
                Daily
              </button>
              <button 
                onClick={() => setReportTimeframe('weekly')}
                className={`px-3 py-2 rounded-md transition-all cursor-pointer ${reportTimeframe === 'weekly' ? 'bg-white text-amber-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
              >
                Weekly
              </button>
              <button 
                onClick={() => setReportTimeframe('monthly')}
                className={`px-3 py-2 rounded-md transition-all cursor-pointer ${reportTimeframe === 'monthly' ? 'bg-white text-amber-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
              >
                Monthly
              </button>
            </div>
        ...
        {/* Live Metrics Grid Layout Rows */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Interval Revenue</p>
              <h4 className="text-2xl font-black text-gray-900">NPR {liveReport.revenue.toLocaleString()}</h4>
            </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div className="space-y-1">
              ...
              <h4 className="text-2xl font-black text-gray-900">{liveReport.itemsSold} Units</h4>
            </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">New Items Added</p>
              <h4 className="text-2xl font-black text-gray-900">{liveReport.itemsAdded} Products</h4>
            </div>
        ...
        {/* DYNAMIC ITEM-BY-ITEM DETAILED SALES QUANTITY BREAKDOWN */}
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-gray-800 uppercase tracking-wider flex items-center gap-1.5">
              <Layers size={14} className="text-amber-500" /> Itemized Performance Breakdown (All Catalog Products)
            </h4>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-gray-100 text-gray-400 font-medium bg-gray-50/50">
                    ...
                    <th className="py-2.5 px-3 text-center">Total Quantities Dispatched</th>
                    <th className="py-2.5 px-3 text-right">Revenue Yielded</th>
                  </tr>
                </thead>
                <tbody>
                  {liveReport.breakdown.map((item, index) => (
                    <tr key={index} className="border-b border-gray-50 text-gray-700 font-medium hover:bg-gray-50/40">
                      ...
                      <td className="py-3 px-3 text-center text-amber-600 font-extrabold">{item.unitsSold}</td>
                      <td className="py-3 px-3 text-right text-emerald-600 font-bold">NPR {item.revenue.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
